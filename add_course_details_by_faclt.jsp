...
<input type="text" placeholder="Total No of Lecture" name="lec" required /><br/>
...
<%   
  ...
  if(lec.length()>2)   
    {
          out.println("<script>alert('Maximum 2 Digit Are Allowed...')</script>");
        
    }
  else
    {      
         
                 sql="select Semester,Subject_Name from course_details where Semester='"+ sem +"' && Subject_Name='"+ sub +"'";
                 ResultSet rs=mystmt.executeQuery(sql);
                 
                 if(rs.next())
                 {
                    out.println("<script>alert('Course Details Already Exist...')</script>");
                 }
                 else
                 {
                 sql="insert into course_details values('"+ dept +"','"+ sem +"','"+ sub +"','"+ lec +"')";
                 mystmt.executeUpdate(sql);
                 out.println("<script>alert('Course Details Added Successfully...')</script>");
