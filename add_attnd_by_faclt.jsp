...
var url= "select_attnd_rno.jsp?seme="+str;
...
<div id='state'>
 ...
 <select name="UserID">
   <option>---Select Starting Roll No---</option>
   <% 
        sql="select UserID from login where Institute_type=(select Institute_type from login where UserID='"+ user_type +"') && UserType='Student'";
...
while(rs.next()){%>
 ...
 <select name="last_UserID">
   <option>---Select Ending Roll No---</option>
   <%
        sql="select UserID from login where Institute_type=(select Institute_type from login where UserID='"+ user_type +"') && UserType='Student'";
              rs=mystmt.executeQuery(sql);
while(rs.next()){%>
   ...
   <%}%>
	...
	<input type="date" value="Select Date" name="date"/>	<br/>					
...
<div class="submit-btn">
	<input type="submit" value="Save" name="check">	
...
<%   
       ...
       try {
            ...
            String first_roll=request.getParameter("UserID");
            String last_roll=request.getParameter("last_UserID");
       ...
       {   
            sql="select * from attendance where Roll_No='"+ first_roll +"' && Date='"+ date +"' && Subject='"+ sub +"'";
            ResultSet rs=mystmt.executeQuery(sql);
    
            if(rs.next())
             {
                    out.println("<script>alert('Record Already Exist...')</script>");
             }
            else
             {
               ...
               if(num.equals(num2))
                {
                    for(int i=0;i<=ans;i++)
                    {
                       ...
                       sql="insert into attendance values('"+ dept +"','"+ sem +"','"+ UserID +"','"+ sub +"','"+ date +"','"+ status +"')";
                       mystmt.executeUpdate(sql);
                      }
                    out.println("<script>alert('Attendance Recorded Successfully...')</script>");
               ...
               if(num.equals(num2))
                {
                    for(int i=0;i<=ans;i++)
                    {
                        ...
                        sql="update attendance set Department_name='"+ dept +"',Semester='"+ sem +"',Roll_No='"+ UserID +"',Subject='"+ sub +"',Date='"+ date +"',Status='"+ status +"' where Roll_No='"+ UserID +"' && Subject='"+ sub +"' && Date='"+ date +"'";
                        mystmt.executeUpdate(sql);
                    
                    }
                  out.println("<script>alert('Attendance Updated Successfully...')</script>");
